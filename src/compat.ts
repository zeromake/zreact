...
const COMPONENT_WRAPPER_KEY = typeof Symbol !== "undefined" ? Symbol.for("__zreactCompatWrapper") : "__zreactCompatWrapper";
...
function render(vnode, parent, callback?: () => void) {
    ...
    if (parent) {
        parent._zreactCompatRendered = out && ((out._vdom && out._vdom.component) || { base: out });
    }
    if (typeof callback === "function") {
        callback();
    }
    return out && out._vdom && out._vdom.component || out;
}
...
function renderSubtreeIntoContainer(parentComponent, vnode, container, callback) {
    const wrap = h(ContextProvider, { context: parentComponent.context }, vnode);
...
function unmountComponentAtNode(container) {
    ...
    if (existing && existing.parentNode === container) {
        zreactRender(h(EmptyComponent), container, existing);
        return true;
    }
    return false;
}
...
function upgradeToVNodes(arr, offset?: number) {
...
function cloneElement(element, props, ...children) {
    ...
    return normalizeVNode(zreactCloneElement(...cloneArgs));
}
...
function multihook(hooks, skipDuplicates?: boolean) {
    return function() {
        let ret;

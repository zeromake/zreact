...
export function createContext<T>(defaultValue: T, calculateChangedBits?: (old: T, now: T) => number): typeof IProvider {
    ...
    class Consumer extends Component<IBaseProps, IConsumerState<T>> implements IConsumer<T> {
        ...
        constructor(props: IBaseProps) {
            ...
            const instance = (Provider.getContext(fiber).stateNode as Provider | null);
            if(instance) {
    ...
    class Provider extends Component<IProviderProps<T>, IProviderState<T>> implements IProvider<T> {
        ...
        public static getContext(fiber: IFiber, now?: IFiber): IFiber | null {
            while (fiber.return){
                if (fiber.type === Provider && fiber !== now){
                    return fiber;
                }
                fiber = fiber.return;
            }
        ...
        public componentWillUnmount() {
            if(this.subscribers.length) {
                this.subscribers.forEach(fiber => {
                    const old = fiber;

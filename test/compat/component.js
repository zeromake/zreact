...
describe('components', () => {
	...
	it('should alias props.children', () => {
        ...
        expect(foo).to.exist;
        expect(foo.props).have.property('children').eql(children);
	...
	describe('refs', () => {
		it('should support string refs', () => {
            ...
            expect(inst.refs).to.exist;

			expect(inst.refs).to.have.property('top', scratch.firstChild._vdom);
			expect(inst.refs).to.have.property('h1', scratch.querySelector('h1')._vdom);
			expect(inst.refs).to.have.property('p', scratch.querySelector('p')._vdom);
			expect(inst.refs).to.have.property('span', scratch.querySelector('span')._vdom);
			...
			expect(innerInst.refs).to.have.property('contained', scratch.querySelector('.contained')._vdom);
			expect(innerInst.refs).to.have.property('inner-h2', scratch.querySelector('h2')._vdom);
		});
		...
		it('should retain support for function refs', () => {
			...
			expect(ref1).to.have.have.been.calledOnce.and.calledWith(scratch.firstChild._vdom);
			expect(ref2).to.have.have.been.calledOnce.and.calledWith(scratch.querySelector('h1')._vdom);
		...
		it('should support string refs via cloneElement()', () => {
			...
			expect(outer).to.have.property('refs').eql({ one: one._vdom });
			expect(inner).to.have.property('refs').eql({ two: two._vdom });
		});
	});

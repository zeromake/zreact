		...
		it('should be called after children are mounted', () => {
			...
			class Inner extends Component {
				componentDidMount() {
					...
					// callback is invoked.
					expect(scratch.querySelector('#inner')).to.equal(findDOMNode(this));
				}
		...
		it('should remove this.base for HOC', () => {
			let createComponent = (name, fn) => {
				class C extends Component {
					componentWillUnmount() {
						expect(findDOMNode(this), `${name}.componentWillUnmount`).to.exist;
						setTimeout( () => {
							expect(findDOMNode(this), `after ${name}.componentWillUnmount`).not.to.exist;
						}, 0);
					}
